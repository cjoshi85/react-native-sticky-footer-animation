...
import { SafeAreaView, ScrollView, Image, Animated, Text, View, Dimensions, StyleSheet, StatusBar } from 'react-native';
...
export default () => {
  const [bottomAction, setBottomAction] = React.useState(null);
  const scrollY = React.useRef(new Animated.Value(0)).current;
  console.log({bottomAction,height});
          ...
          {articleParagraphs.slice(0, 3).map((item, index) => {
            return (
              <View key={`featured-${index}`} style={{flexDirection: 'row', marginBottom: 10}}>
                ...
                <Text numberOfLines={3} style={styles.paragraph}>{item}</Text>
              </View>
            )
          })}
        </View>
      </Animated.ScrollView>
      {bottomAction?<Animated.View style={[styles.bottomActions, {
        ...
        <View style={{flexDirection: 'row', height: 60, alignItems: 'center', justifyContent: 'center'}}>
          <Entypo name='adjust' size={24} color='white' style={{marginHorizontal: 10}}/>
          <Text>326</Text>
        </View>
